...
C
C
C A Real array (My N) input: the data matrix
C M Integer input : the number of points
C N Integer input: the number of dimensions
C C Real array (K, N) input: the matrix of initial cluster centres
...
C AN1 Real array (K) workspace :
C AN2 Real array (K) workspace :
C
C NCP Integer array (K) workspace :
C D Real array (M) workspace :
...
C WH para los pesos
C
      SUBROUTINE KMNSW(A, M, N, C, WH, K, IC1, IC2, NC, AN1, AN2, NCP,
...
C
      ...
      DOUBLE PRECISION WH(M) , WHC(K)
C     WH pesos de los individuos y WHC pesos de los cluster
C     la suma de los pesos no necesariamente es 1
C
...
C ---------------------------------
        DO 80 J = 1, N
   80   C(L,J) = C(L,J) + A(I,J)*WH(I)
C                                peso del individuo
   90 CONTINUE
C     --------------------------------------------------------------
...
C
C ---------------------------------
C        AN2(L) = AA / (AA + ONE)  se cambia por
        AN2(L) = WAA / (WAA + WAA/AA)
C ---------------------------------
        AN1(L) = BIG
C ---------------------------------
C        IF (AA .GT. ONE) AN1(L) = AA / (AA - ONE)
        IF (AA .GT. ONE) AN1(L) = WAA / (WAA - WAA / AA)
C ---------------------------------
        ITRAN(L) = 1
        NCP(L) = -1
  120 CONTINUE
...
C  WH (pesos de individuos) y WHC (peso de clusters, para FactoClass
        CALL OPTRA(A, M, N, C, WH, WHC, K, IC1, IC2, NC, AN1, AN2,
...
C     ################################ LLAMADO QTRAN
C  WH (pesos de individuos) y WHC (peso de clusters, para FactoClass
        CALL QTRAN(A, M, N, C, WH, WHC, K, IC1, IC2, NC, AN1, AN2,
     *        NCP, D, ITRAN, INDX)
C
...
C
  ...
  160 CONTINUE
      DO 170 I = 1, M
        II = IC1(I)
        DO 170 J = 1, N
          C(II,J) = C(II,J) + A(I,J)*WH(I)
C                                    peso del punto
  170 CONTINUE
      DO 190 J = 1, N
        DO 180 L = 1, K
C -------------------------------------
C  180   C(L,J) = C(L,J) /  FLOAT(NC(L)) se cambia por
  180   C(L,J) = C(L,J) / WHC(L)
          DO 190 I = 1, M
          II = IC1(I)
          DA = A(I,J) - C(II,J)
          WSS(II) = WSS(II) + DA*DA*WH(I)
C                                   peso del punto
  190 CONTINUE
C
      RETURN
...
C            C(L2,J) = (C(L2,J) * AL2 + A(I,J)) / ALT
            C(L1,J) = (C(L1,J) * WAL1 - A(I,J)*WH(I)) / WALW
            C(L2,J) = (C(L2,J) * WAL2 + A(I,J)*WH(I)) / WALT
C ------------------------------
   80     CONTINUE
C ------------------------------
C         Nuevos pesos de las clases
          WHC(L1) = WAL1
          WHC(L2) = WAL2
C         ---------------------
          NC(L1) = NC(L1) - 1
...
C          AN2(L1) = ALW / AL1
          AN2(L1) = WALW / WAL1
          AN1(L1) = BIG
C          IF (ALW .GT. ONE) AN1(L1) = ALW / (ALW - ONE)
          IF (ALW .GT. ONE) AN1(L1) = WALW / (WALW - WH(I))
C          AN1(L2) = ALT / AL2
          AN1(L2) = WALT / WAL2
C          AN2(L2) = ALT / (ALT + ONE)
          AN2(L2) = WALT / (ALT + WH(I))
          IC1(I) = L2
          IC2(I) = L1
   90   CONTINUE
        IF (INDX .EQ. M) RETURN
...
C antepuestos de W son nuevos
        ...
        ALT = AL2 + ONE
        WALT = WAL2 + WH(I)
C         Centros generados a partir de las ponderaciones
        DO 50 J = 1, N
          C(L1,J) = (C(L1,J) * WHC(L1) - A(I,J)*WH(I)) / WALW
          C(L2,J) = (C(L2,J) * WHC(L2) + A(I,J)*WH(I)) / WALT
   50   CONTINUE
C         Nuevos pesos de las clases
          WHC(L1) = WALW
          WHC(L2) = WALT
